 ...
 *-----------------------------------------------------------------*/
const char* inputFile = "output.cod";
const char* encdecFile = "encdec.wav";
int main(int argc, char *argv[] )
{
  Word16  synth_buf[L_FRAME+M], *synth; /* Synthesis                   */
  Word16  parm[PRM_SIZE+1] = {0};             /* Synthesis parameters        */
  Word16  serial[SERIAL_SIZE] = {0};          /* Serial stream               */
  ...
  if ( (f_serial = fopen(inputFile, "rb")) == NULL) {
     printf("Error opening file  %s !!\n", inputFile);
     exit(0);
  }
  printf("Linear file    :  %s\n", inputFile);

  if ( (f_syn = fopen(encdecFile, "wb")) == NULL) {
     printf("Error opening file  %s !!\n", encdecFile);
     exit(0);
  }
   /* Passed arguments */
 ...
 *-----------------------------------------------------------------*/
  ...
  while( fread(&parm[1], sizeof(Word16), PRM_SIZE, f_serial) == PRM_SIZE)
  {
